...
import { listarCampos, criarCampo, atualizarCampo, deletarCampo } from '../services/campoService'
import Navbar from '../components/Navbar'

const FORM_VAZIO = { nome: '', endereco: '', descricao: '', preco: '', horaAbertura: '', horaFechamento: '' }
...
function MeusCampos() {
  ...
  async function carregarCampos() {
    try {
      ...
      setCampos(dados.filter((c) => c.donoId === usuario.id))
    } catch (error) {
  ...
  function iniciarEdicao(campo) {
    ...
    setFormEdicao({
      ...
      preco: campo.preco,
      horaAbertura: campo.horaAbertura,
      horaFechamento: campo.horaFechamento
    })
    setMensagem('')
  }
          ...
          <form onSubmit={handleCriar} className="bg-white rounded-2xl shadow-md p-6 mb-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="time"
                  value={formNovo.horaAbertura}
                  onChange={(e) => setFormNovo({ ...formNovo, horaAbertura: e.target.value })}
                  required
                  className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-verde-500"
                />
              </div>
              <div>
                ...
                <input
                  type="time"
                  value={formNovo.horaFechamento}
                  onChange={(e) => setFormNovo({ ...formNovo, horaFechamento: e.target.value })}
                  required
                  className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-verde-500"
                />
              </div>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        Funciona das {campo.horaAbertura} às {campo.horaFechamento}
                      </p>
                    </div>
