...
function Campos() {
  ...
  const [campoAgendandoId, setCampoAgendandoId] = useState(null)
  const [form, setForm] = useState({ data: hoje, horaInicio: '', horaFim: '' })
  ...
  function iniciarAgendamento(campo) {
    setCampoAgendandoId(campo.id)
    setForm({ data: hoje, horaInicio: campo.horaAbertura, horaFim: campo.horaFechamento })
    setMensagem('')
  }

  async function handleAgendar(e, campoId) {
    e.preventDefault()
    setMensagem('')

    try {
      await criarAgendamento({ campoId, ...form })
      setMensagem('Agendamento criado! Acompanhe em Meus Agendamentos.')
      setCampoAgendandoId(null)
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Funciona das <span className="font-medium">{campo.horaAbertura}</span> às{' '}
                      <span className="font-medium">{campo.horaFechamento}</span>
                    </p>
                  </div>

                  {usuario?.perfil === 'JOGADOR' && (
                    <button
                      onClick={() => campoAgendandoId === campo.id ? setCampoAgendandoId(null) : iniciarAgendamento(campo)}
                      className="text-xs bg-verde-700 text-white px-3 py-1.5 rounded hover:bg-verde-800 shrink-0"
                    >
                      {campoAgendandoId === campo.id ? 'Cancelar' : 'Agendar'}
                    </button>
                  )}
                </div>

                {campoAgendandoId === campo.id && (
                  <form onSubmit={(e) => handleAgendar(e, campo.id)} className="mt-4 border-t border-gray-100 pt-4 flex items-end gap-3 flex-wrap">
                    <div>
                    ...
                    <div>
                      ...
                      <input
                        type="time"
                        min={campo.horaAbertura}
                        max={campo.horaFechamento}
